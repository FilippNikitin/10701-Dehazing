...
class ESDNet(nn.Module):
    def __init__(self,
        ...
        self.last_conv = self.conv_first = nn.Sequential(
            nn.Conv2d(3, in_chans, kernel_size=1, stride=1, padding=0, bias=True),
            nn.Tanh()
        )
...
class Encoder(nn.Module):
    def __init__(self, feature_num, inter_num, sam_number, input_channels=3):
        super(Encoder, self).__init__()
        self.conv_first = nn.Sequential(
            nn.Conv2d(input_channels*4, feature_num, kernel_size=5, stride=1, padding=2, bias=True),
